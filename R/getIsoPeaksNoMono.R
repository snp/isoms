...
#'
#' @examples
get_isopeaks_nomono <- function(ss,
                         mz,
                         width = 0.002,
                         npoint = 10,
                         tol = 0.01,
  ...
  if (class(model) != 'try-error' & r2 > 0.9) {
    ...
    res <- tidy(model) %>%
      ...
      mutate(
        R2 = r2,
        isoshift=v['mu'] - mz,
        theorshift = isoshift_C,
        masserror = v['mu'] - mz - isoshift_C,
        isoratio = 1)
          ...
          if (class(model_) != 'try-error' & r2_ > 0.5) {
            ...
            res <-
              res %>% bind_rows(tidy(model_) %>%
                                  ...
                                  mutate(
                                    R2 = r2_,
                                    isoshift = v_['mu'] - v['mu']+isoshift_C,
                                    theorshift = isoshift_,
                                    masserror = v_['mu'] - v['mu'] +isoshift_C - isoshift_,
                                    isoratio = v_['I'] / v['I']))
          }
        }
      }
