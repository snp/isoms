#' Isotopic peaks fitting for specified m/z (only +1 isotopes, no mono)
#'
#' Petforms peak fitting at m/z values
#' corresponding to the 13C, 15N, 2H and 18O isotopes relative to provided m/z.
#' Returns data_frame
...
#'
#' @examples
get_isopeaks_nomono <- function(ss, mz, width = 0.002, npoint = 10, tol = 0.01, fixSigma = T) {
    # Checking input data
    ...
    element_ = "C"
    isotope_ = "13C"
    isoshift_C = diff((isotopes %>% filter(element == element_) %>% filter(abundance > 0.9 | isotope == isotope_))$mass)
    isomz_ <- mz + isoshift_C
    
    p_int <- findInterval(c(isomz_ - tol, isomz_ + tol), ss[, 1])
    if (diff(p_int) < npoint) {
        warning(sprintf("No peak found at mz=%.4f with tol=%.4f", isomz_, tol))
        return(data.frame())
    }
    ...
    # plot(sub_ss) xx = seq(min(sub_ss[,1]), max(sub_ss[,1]),length.out=400) yy = predict(model, newdata=list(x=xx)) lines(xx,yy)
    if (class(model) != "try-error" & r2 > 0.9) {
        v <- coef(model)
        res <- tidy(model) %>% mutate(peak = "13C") %>% select(peak, term, estimate) %>% spread(term, estimate) %>% mutate(R2 = r2, isoshift = v["mu"] - mz, 
            theorshift = isoshift_C, masserror = v["mu"] - mz - isoshift_C, isoratio = 1)
        for (isotope_ in c("15N", "2H")) {
            element_ = sub("\\d+", "", isotope_)
            isoshift_ = diff((isotopes %>% filter(element == element_) %>% filter(abundance > 0.9 | isotope == isotope_))$mass)
            isomz <- v["mu"] - isoshift_C + isoshift_
            ...
            if (diff(p_int) > npoint) {
                ...
                if ((iend - istart + 1) > npoint) {
                  ...
                  model_ = fitPeak(sub_ss[, 1], sub_ss[, 2], startPars = startPars, maxAttempts = 100, sigma = ifelse(fixSigma, v["sigma"], 0))
                  r2_ = rSquared(model_, sub_ss[, 2])
                  # plot(sub_ss)
                  if (class(model_) != "try-error" & r2_ > 0.5) {
                    # xx = seq(min(sub_ss[,1]), max(sub_ss[,1]),length.out=400) yy = predict(model_C, newdata=list(x=xx)) lines(xx,yy)
                    v_ = coef(model_)
                    res <- res %>% bind_rows(tidy(model_) %>% mutate(peak = isotope_) %>% select(peak, term, estimate) %>% spread(term, estimate) %>% mutate(R2 = r2_, 
                      isoshift = v_["mu"] - v["mu"] + isoshift_C, theorshift = isoshift_, masserror = v_["mu"] - v["mu"] + isoshift_C - isoshift_, isoratio = v_["I"]/v["I"]))
                  }
                }
            }
        }
