quantifyIsoRatio <- function(data, control = "Control", file = "20171226_02_NP02_fit.csv", peak = "13C", ion = "P") {
    ...
    span_ = 0.2
    degree_ = 1
    loess_control <- loess(gamma ~ ldI, data = s_control, weights = ltic, span=span_, degree = degree_)
    s_control$loess <- loess_control$fitted
    loess_sample <- loess(gamma ~ ldI, data = s_sample, weights = ltic, span=span_, degree = degree_)
